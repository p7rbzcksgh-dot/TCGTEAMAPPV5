...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if ('serviceWorker' in navigator) navigator.serviceWorker.register('sopwizard__service-worker.js').catch(() => {});
});
...
function saveDraft(){
  ...
  try {
    ...
    history.replaceState(null, '', 'sopwizard__create-sop.html?id=' + encodeURIComponent(currentDraftId));
...
function renderSavedList(){
  ...
  if(!drafts.length){
    list.innerHTML = `<div class="empty-state glass-card"><img src="sopwizard__tcg-cogs-branding.png" alt=""><h2>No saved SOPs yet</h2><p>Create a new SOP and tap Save Draft. Drafts stay on this device until exported or browser data is cleared.</p><a class="glow-btn" href="sopwizard__create-sop.html">${icons.plus} Create A New SOP</a></div>`;
    return;
  }
  list.innerHTML = drafts.map(rec => `<article class="saved-card"><h2>${escapeHTML(rec.sop.title || 'Untitled SOP')}</h2><p>${escapeHTML(rec.sop.mainAssembly || 'SBX')} · ${escapeHTML(rec.sop.department || '')} · ${escapeHTML(rec.sop.revision || '')} · ${new Date(rec.updatedAt).toLocaleString()}</p><div class="saved-actions"><a class="glow-btn" href="sopwizard__create-sop.html?id=${encodeURIComponent(rec.id)}">Open Draft</a><button class="glow-btn" type="button" data-export-saved="${escapeAttr(rec.id)}">Export HTML</button><button class="glow-btn" type="button" data-delete-saved="${escapeAttr(rec.id)}">Delete</button></div></article>`).join('');
